...
5. Gmail 연결은 관리자 UID별로 따로 저장됩니다. 관리자가 발송할 때는 마지막으로 연결한 공용 계정이 아니라 현재 로그인한 관리자가 직접 연결한 Gmail 주소가 발송자가 됩니다.
6. 관리자마다 한 번씩 발송 계정을 연결합니다. 다른 관리자가 Gmail을 연결해도 기존 관리자의 발송 계정은 바뀌지 않습니다.
7. 테스트 급여를 확정해 선생님에게 금액이나 PDF 없이 포털 확인 링크만 발송되는지 확인합니다.
8. 일부 실패 시 `미발송 안내 발송`을 눌러 성공한 선생님에게 중복 발송되지 않고 실패 건만 처리되는지 확인합니다.
...
10. 각 관리자에게 서로 다른 Gmail을 연결하고, 각 관리자가 발송할 때 본인에게 표시된 Gmail 주소가 발송자로 사용되는지 확인합니다.
