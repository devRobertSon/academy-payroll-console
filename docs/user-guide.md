...
`계약 요약`을 먼저 선택하면 선택한 소득 유형에 필요한 영역만 열립니다. 근로소득 또는 혼합형에서는 기본 근로소득 월 지급액을 1원 단위로 입력합니다. 국민연금, 건강보험·장기요양, 고용보험 가입 항목을 선택하면 `예상 보험료`와 예상 소득세·지방소득세가 표시됩니다. 선생님 관리 상세에서도 선택된 급여월의 기본 급여 기준 보험료와 합계를 확인할 수 있습니다. 보험료 자동 계산은 근로소득과 보험 적용 대상 수당을 따르며, 기존에 따로 저장한 보험 신고 기준액은 새 계산에서 사용하지 않습니다. 세 보험은 실제 가입 상태와 적용 기간에 맞게 따로 관리합니다. 산재보험은 사업주 부담이므로 근로자 공제액에는 포함하지 않습니다. 실제 공단 신고를 변경하는 기능은 아니며, 공단 고지액과 다른 공제액은 급여 확정 전 `과세·공제 조정`에서 월별로 맞춥니다. 기존 수동 공제액과 이미 발행된 명세서는 유지됩니다.
...
5. 관리자는 `월 급여 입력`에서 해당 선생님 행을 눌러 선택한 뒤 다시 눌러 제출 내용을 확인하거나 직접 입력합니다. 환불·할인 등은 계약상 정산 기준에 따라 총액에 반영하고, 비율 정산 대상이 아닌 수업은 포함하지 않습니다.
...
관리자는 `선생님 관리`에서 선생님을 선택하고 상세 정보 아래의 `DC형 퇴직연금`을 확인합니다. 이 정보는 선생님 화면, 급여명세서·PDF와 급여 엑셀에 포함되지 않습니다.

1. 가입 대상인지 확인한 뒤 `퇴직연금 대상`을 체크합니다. 체크만으로 가입이나 은행 납입이 실행되지는 않습니다.
2. `대상 급여월`을 선택하고 `근거 확인 · 납입액 확정`을 누릅니다. 미확정 급여는 예상액만 검토할 수 있으며 실제 납입액 확정은 급여 발행 후 가능합니다.
3. 지급 항목별 세전 금액과 원래 급여 산정 근거를 확인합니다. 기본 급여·강사료는 우선 포함하고, 교통비·기타비·영수증 금액은 우선 제외합니다. 실제 임금에 해당하는 수당은 포함하고 실비변상은 제외하도록 관리자가 검토합니다. 소득 구분이나 비과세 여부만으로 판단하지 않습니다.
4. 예상액은 `산입 대상 세전 임금 합계 ÷ 12`이며 원 미만은 올립니다. 예를 들어 2,000,000원이 모두 산입되면 예상액은 166,667원입니다. 포함 항목을 바꾸면 합계와 예상액도 바뀝니다.
5. 은행 납입 내역을 확인하고 `실제 납입액`과 `납입 확인일`을 직접 입력합니다. 실제 금액은 자동 입력되지 않습니다. 산입 항목을 변경했거나 예상액과 실제 금액이 다르면 메모에 사유를 적습니다. 미납입을 기록할 때는 0원과 사유를 입력합니다.
6. 검토 확인란을 체크하고 `납입액 확정`을 누릅니다. 예상액, 계산 근거, 실제 금액, 차이, 처리자와 확정 시각이 함께 보관됩니다.
7. 수정이 필요하면 `납입 확정 내역 → 납입액 정정`에서 5자 이상의 정정 사유와 수정값을 입력합니다. `확정 이력`으로 이전 차수의 금액과 당시 계산 근거를 확인할 수 있습니다. 급여 취소·재발행만으로 기존 납입 기록이 바뀌지는 않습니다.

월 예상액은 관리용 추정치입니다. DC형의 연간 최소 부담금은 연간 임금총액의 1/12이며, 월별 올림·상여·휴직·휴업·중도 입퇴사 등을 반영한 연간 정산과 규약상 납입 주기 확인이 필요합니다. 대상 여부와 특수기간 처리 기준은 퇴직연금 사업자 또는 노무사에게 확인합니다. [고용노동부 DC형 부담금 안내](https://www.moel.go.kr/minwon/fastcounsel/fastcounselView.do?inetDcssMngId=202505120240591690507)

![가상 자료로 확인한 DC형 퇴직연금 계산 근거와 납입액 정정 화면](./images/guide-04-retirement.png)

## 5. 월 급여 입력

1. `월 급여 입력`에서 급여월을 선택합니다.
2. 상단 종 아이콘에서 수업시간 제출 알림은 해당 선생님의 월 지급액 입력 창을, 영수증 제출 알림은 해당 영수증 검토 창을 엽니다.
3. 각 선생님의 보험 가입, 이번 달 근로소득과 등록된 사업소득 시급·약정 비율을 확인합니다.
4. 표는 `선생님 → 실 지급액 → 신고액 → 가입 보험` 순서로 시작하며 실 지급액은 공제 후 금액입니다. 선생님 행의 칸을 한 번 누르면 선택되고, 선택된 행을 다시 누르면 월 지급액 입력 창이 열립니다. 다른 행을 누르면 선택만 이동합니다. 키보드에서는 선생님 칸에 Tab으로 이동한 뒤 Enter 또는 Space를 같은 순서로 사용합니다. 제출된 수업시간과 해당 수업 전체 학원비를 확인하고 누락이나 오류를 수정합니다. 관리자가 저장한 학원비는 선생님 재제출로 덮어쓰지 않으며, 필요한 경우 `선생님 제출값 반영`을 누르고 저장합니다.
5. 근로소득 계약은 근로소득과 근로 수업시간만, 사업소득 계약은 사업소득만 표시하고 혼합 계약은 둘 다 표시합니다. 사업소득도 등록된 시급·비율·혼합에 맞는 항목만 표시합니다. 월급·시급·약정 비율은 읽기 전용이고 월별 시간과 해당 수업 전체 학원비만 입력합니다. 보험 신고 기준액은 별도로 입력하지 않고 근로소득에 연동됩니다.
...
월급·시급·약정 비율 등 계약 정보를 바꾸려면 월 지급액 창 위의 `선생님 정보 수정`을 누릅니다. 별도 수정 창에서 저장하거나 취소하면 입력하던 월 지급액 창으로 돌아옵니다. 입력 중이던 시간·학원비·교통비·기타 지급·메모는 유지되며, 저장한 새 시급과 비율로 금액을 다시 계산합니다. 계약에서 삭제한 항목은 제외 알림과 함께 빠지고 새 항목은 시간 0 또는 학원비 입력 대기로 추가됩니다. 선생님 정보는 그 창에서 즉시 저장되지만 월 입력 내용은 돌아온 창에서 `저장`해야 반영됩니다.

별도 수정 창에서 기본 월급이나 소득 구성을 변경했다면 돌아온 월 입력 창의 근로소득도 새 계약에 맞게 표시되며, 월 입력을 저장하면 해당 월에도 적용합니다. 계약을 바꾸지 않았다면 기존 월 근로소득 직접값은 그대로 유지합니다. 이미 발행된 명세서는 바꾸지 않습니다.

`교통비를 보험 기준에 포함`은 교통비 지급 여부가 아닌 보험료 산정 대상 여부입니다. 교통비 처리 방식이 `근로소득 과세`이고 이 항목이 체크된 경우에만 자동 보험료 계산 기준에 더합니다. 예를 들어 월 근로소득 2,000,000원과 대상 교통비 100,000원이면 기준은 2,100,000원입니다. `비과세 실비` 등 다른 처리 방식은 체크해도 이 기준에 더하지 않습니다. 실제 지급 성격에 맞는 처리는 세무사와 확인합니다.
...
![근로소득이 고정된 월 지급액 입력 창](./images/guide-05-monthly-form.png)
...
5. H열은 강사료 원천징수 합계, L열은 교통비·기타 지급의 별도 원천징수 합계입니다. R·S열 근로소득세·지방소득세는 관리자 참고액으로만 반영하고 공제하지 않습니다. T~W열은 보험별 공제액입니다. H·L은 계속 공제하므로 근로소득 참고 세금을 이 열에 중복 입력하지 않습니다. 혼합 소득의 L열 공제는 어느 명세서에 반영할지 선택합니다.
...
8. 반영된 금액은 `엑셀 직접 입력`으로 표시됩니다. 기존 시급·비율제 산정 근거와 선생님 제출값은 보존됩니다. 선생님 행을 선택한 뒤 다시 누르면 소득 구분에 맞는 직접 입력 항목을 표시합니다. 근로소득은 읽기 전용이며 `선생님 정보 수정` 창으로 이동했다 돌아와도 입력 중이던 직접값은 유지됩니다. 엑셀 직접 입력 강사료는 시간·학원비로 산출한 금액이 아니므로 시급·비율을 바꿔도 자동 재산정하지 않습니다. 산정 방식으로 돌아가려면 직접 입력 해제 후 수업 내역을 확인합니다. 엑셀 불러오기로 금액을 반영하는 절차는 기존대로 변경 전후를 확인합니다. 확정·공개·메일 발송은 자동 실행되지 않습니다.
...
새 계산의 엑셀 내보내기는 실제 공제액을 따르므로 R·S열이 0원입니다. 관리자 세금 참고액은 내보내지 않으며, 이 파일을 다시 불러오면 R·S의 0원도 직접 참고액으로 반영되므로 확인합니다. 이미 발행된 명세서는 발행 당시 공제액을 그대로 내보냅니다.
...
새 급여 계산에서는 **근로소득세와 근로소득 지방소득세를 공제하지 않습니다.** `선생님 관리` 상세와 관리자 명세서 화면의 `근로소득 세금 참고액`에서 금액과 계산 기준을 확인합니다. 간이세액표 또는 관리자·엑셀의 직접 입력값을 사용하며 `과세·공제 조정`에서 참고액을 조정할 수 있습니다. 사업소득 3.3%, 기타소득 세금, 엑셀 H·L 직접 공제, 사회보험료와 기타 공제는 그대로 적용됩니다.

관리자 참고액은 선생님 화면·급여명세서 본문·PDF·인쇄·메일 첨부·급여 내보내기에 포함하지 않습니다. 이미 발행한 명세서의 금액은 자동 변경하지 않으므로 새 방식을 적용하려면 검토 후 확정 취소·재발행이 필요합니다. 포털에서 공제를 제외해도 법정 원천징수·납부 의무가 없어지는 것은 아니므로 실제 세금 부담과 신고는 세무사와 확인합니다.
...
근로소득세·지방소득세 열과 CSV는 참고액이 아닌 실제 급여 공제액을 표시하므로 새 계산에서는 0원입니다. 이전에 발행한 명세서는 당시 값을 유지합니다. 별도 참고액은 관리자 명세서 화면이나 선생님 관리 상세에서 확인합니다.
...
금액은 1원 단위로 입력할 수 있습니다. 월 급여 입력의 근로소득은 읽기 전용이고 보험료 계산 기준은 자동 연동됩니다. 자동 계산에서는 국민연금 기준소득월액의 천원 미만을 먼저 버리고, 국민연금·건강보험·장기요양·고용보험 공제액의 10원 미만을 버립니다. 공단 고지액을 수동으로 조정할 때는 1원 단위 입력값이 그대로 적용됩니다.
