...
4. 표는 `선생님 → 실 지급액 → 가입 보험` 순서로 시작하며 실 지급액은 공제 후 금액입니다. 선생님 행의 칸을 한 번 누르면 선택되고, 선택된 행을 다시 누르면 월 지급액 입력 창이 열립니다. 다른 행을 누르면 선택만 이동합니다. 키보드에서는 선생님 칸에 Tab으로 이동한 뒤 Enter 또는 Space를 같은 순서로 사용합니다. 제출된 수업시간과 해당 수업 전체 학원비를 확인하고 누락이나 오류를 수정합니다. 관리자가 저장한 학원비는 선생님 재제출로 덮어쓰지 않으며, 필요한 경우 `선생님 제출값 반영`을 누르고 저장합니다.
