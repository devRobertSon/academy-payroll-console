...
사회보험은 국세청 소관 세금과 분리한 `insurancePolicies/{버전}`으로 관리합니다. 저장소의 사회보험 값은 화면 동작용 데모이므로 실제 운영 전 국민연금공단·국민건강보험공단·근로복지공단 기준과 가입 상태를 확인해야 합니다. 상세 계산 근거와 변경 절차는 [`docs/tax-policy.md`](./docs/tax-policy.md)를 참고하세요.
