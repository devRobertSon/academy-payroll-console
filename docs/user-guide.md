...
선생님은 `등록 정보 > 내 정보 수정`에서 이름, 휴대전화, 생년월일, 국민연금·건강보험·고용보험 가입 여부와 사업소득 시급을 직접 저장할 수 있습니다. 입력칸은 화면 순서대로 `Tab` 키로 이동합니다. 이름에는 한글 또는 영문과 단어 사이 공백만 입력할 수 있습니다. 휴대전화와 생년월일에는 숫자만 입력되며, 휴대전화 네 자리 또는 생년월일 한 자리를 채우면 다음 입력칸으로 자동 이동합니다. 휴대전화의 하이픈은 화면에만 보이며 포털에는 `01012345678`과 같은 숫자 11자리 문자열로 저장됩니다. 생년월일은 앞 여섯 칸과 하이픈 뒤 첫 칸을 차례로 입력하고 나머지 여섯 자리는 검은 점으로 가려집니다. 시급이 하나면 `시급 1개 사용`을 선택하고 금액 하나만 입력합니다. 실제 시급 금액이 여러 개일 때만 `여러 시급 사용`을 선택하고 `시급 1`, `시급 2` 항목에 금액을 2개 이상 입력합니다. 과목명은 입력하지 않습니다.
