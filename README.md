...
학원 선생님별 근로소득 월급, 한 개 또는 여러 개의 사업소득 시급·수업시간, 보험별 신고 기준액과 추가 지급 항목을 기준으로 급여명세서와 기장용 급여내역서를 만드는 GitHub Pages + Firebase 웹 앱입니다.
...
- 강사료 계산 방식은 `시급 / 비율 / 혼합`입니다. 혼합에서 비율 포함을 체크하면 비율 하나와 최대 9개 시급을 함께 적용하고, 체크하지 않으면 최대 10개 시급을 사용합니다.
- 비율제는 학원 전체 매출이 아니라 **해당 수업 전체 학원비 × 약정 비율**로 계산합니다. 학생 수와 1인당 단가 대신 월별 정산 대상 총액을 한 칸에 입력합니다. 선생님이 제출하거나 비워두고 관리자가 보완할 수 있습니다. 관리자 입력값은 선생님 재제출로 덮어쓰지 않습니다. 미입력과 명시적 0원 정산을 구분하고 명세서·CSV에 전체 학원비와 적용 비율을 보존합니다. 기존 인원별 입력과 발행된 명세서는 삭제하거나 일괄 변경하지 않습니다.
- 비율제 반영 시 포털 배포 전에 최신 `firestore.rules`를 게시해야 합니다. 기존 시급제 데이터, Firebase 설정, Storage 규칙과 Cloudflare Worker 설정은 변경할 필요가 없습니다.

- 관리자/선생님 역할별 화면
- 관리자도 `선생님 관리 > 내 급여 등록`에서 본인 Google 계정에 급여 정보를 연결하고 `관리 업무 / 내 급여` 화면을 전환할 수 있습니다. 계정의 `role: admin`은 유지하며, 선생님 비활성화·삭제로 관리자 접근을 차단하지 않습니다. 최신 Firestore 규칙 재게시가 필요하며 Worker·Storage·OAuth 설정은 변경하지 않습니다.
...
- 개인정보 입력 후 계약 요약을 근로소득·사업소득·혼합형으로 선택하고 유형별 입력 화면 자동 전환
- 기본 근로소득 월급과 한 개/여러 개 사업소득 시급 관리
- 선생님 정보 수정·비활성화와 계정 접근 동시 제어
- 이메일·월급·보험 신고 기준액·세금·공제 정보의 관리자 전용 수정
- 월별 근로소득 월급과 시급 항목별 사업소득 수업시간 입력
- 수업이 없는 보험 가입자의 월급 계산
- 사업소득의 시급 항목별 금액 × 수업시간 및 3.3% 원천징수 자동 계산
- 교통 횟수·교통비(주차비 포함)·기타 지급 입력과 지급 항목별 과세 처리
...
사회보험은 국세청 소관 세금과 분리한 `insurancePolicies/{버전}`으로 관리합니다. 앱에는 2026년 공식 근로자 부담률과 국민연금·건강보험 상하한이 시행일별로 내장되어 있으며, 관리자는 `계산 · 보안 설정`에서 새 버전을 추가할 수 있습니다. 신고 기준액은 1원 단위로 입력하되 국민연금 기준소득월액의 천원 미만과 자동 보험료의 10원 미만을 절사합니다. 자동 계산은 예상값이므로 실제 확정 전 공단 고지액, 입·퇴사월, 두루누리 지원, 휴직·정산 내역을 확인하고 필요한 경우 선생님별 1원 단위 수동 공제액을 사용합니다. 상세 계산 근거와 변경 절차는 [`docs/tax-policy.md`](./docs/tax-policy.md)를 참고하세요.
