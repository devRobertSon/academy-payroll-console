...
`공통 지급 설정`의 `기타 기본금액`은 매월 기본 지급액에 포함됩니다. 과세 처리와 보험 신고 기준 포함 여부를 확인하세요. 월 급여 입력에 이미 저장한 기타 항목이나 엑셀 직접 입력이 있으면 그 월의 값이 우선하며, 기타 항목을 모두 지우고 저장하면 그 월은 0원으로 처리됩니다. 엑셀 J열에는 교통비와 주차비 합계, K열에는 기타비만 표시됩니다.
...
9. `엑셀 내보내기`는 미확정 월의 활성 선생님 전체를 원본과 같은 A~X열 형식으로 저장합니다. 사이트에서는 교통비·주차비·기타비를 따로 관리하고 엑셀에서만 J열에 교통비와 주차비를 합산하며, K열에는 기타비만 넣습니다. 확정된 월은 이후 비활성화된 선생님도 포함해 당시 확정 명세서의 금액을 내보냅니다. 검색 필터와 관계없이 전체가 포함됩니다. 개인정보 열인 주민등록번호·주소·은행·계좌번호는 불러오지 않고 내보낼 때도 빈칸입니다.
