...
`incomeComposition`은 소득 구성을 저장합니다. `employee`는 근로소득 설정만, `business`는 사업소득 설정만, `mixed`는 두 설정을 모두 급여 계산에 사용합니다. `contractSummary`는 표준 한글 표시값입니다. `insuranceSettings`는 국민연금·건강보험(장기요양 포함)·고용보험의 가입 여부, 기본 신고 기준액과 적용 기간을 관리합니다. 선생님은 가입 여부만 바꿀 수 있고 신고 기준액·기간은 관리자 값이 보존됩니다. `defaultEmployeePay`는 관리자 전용 근로소득 기본 월급입니다. `usesSubjectRates`가 `false`이면 `defaultBusinessHourlyRate`와 고정 ID `default-business-rate`를 사용하며 과목명을 받지 않습니다. `true`이면 `businessRates`에 시급이 다른 과목을 2개 이상 저장합니다. 기존 단일 과목 시급은 수정 화면에서 기본 시급으로 전환할 수 있습니다.

선생님 본인은 `name`, `phone`, `birthDateCode`, `genderCode`, 보험 가입 여부, 기본/과목별 시급과 그에 따른 소득 구성을 수정할 수 있습니다. 과목별 시급 목록은 최대 10개이며 이름·시급 형식을 규칙에서 검사합니다. `email`, `authUid`, `status`, 보험 신고 기준액·적용 기간, 근로소득 월급, 교통비 기준, 지급일과 원천징수 정보는 관리자만 수정할 수 있습니다.
...
이 문서에는 급여액, 시급, 소득 구분이나 보험 정보가 없습니다. `businessHours`의 키는 선생님 문서의 유효 시급 항목 ID입니다. 기본 시급은 `default-business-rate`, 과목별 시급은 각 `businessRates.id`를 사용합니다. 계산 시 등록된 ID의 시수만 사용하며 알 수 없는 키는 무시합니다. 선생님은 본인 UID·teacherId의 문서만 읽고 쓸 수 있고, `payrollRuns/{month}`가 `published`이면 쓰기가 거부됩니다.
